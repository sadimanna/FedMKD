...
def read_data(dataset_name, train_data_dir, test_data_dir, public_data_dir):
    ...
    Args:
        ...
        public_data_dir (str): The directory of public data.
    ...
    _, _, public_data = read_dir(public_data_dir)

    assert train_clients == test_clients
    assert train_groups == test_groups

    return train_clients, train_groups, train_data, test_data, public_data
...
def load_data(root,
    ...
    setting = BaseDataset.get_setting_folder(dataset_name, split_type, num_of_clients, min_size, class_per_client,
                                             data_amount, iid_fraction, user_str, train_test_split, public, alpha,
                                             quantity_weights)
    ...
    public_data_dir = os.path.join(data_dir, setting, "public")

    if not os.path.exists(train_data_dir) or not os.path.exists(test_data_dir) or not os.path.exists(public_data_dir):
    ...
    users, train_groups, train_data, test_data, public_data = read_data(dataset_name, train_data_dir, test_data_dir, public_data_dir)

    return train_data, test_data, public_data, process_x, process_y, transform_train, transform_test
...
def construct_datasets(root,
    ...
    Args:
        ...
        quantity_weights (list[float]): The targeted distribution of quantities to simulate data quantity heterogeneity.
            ...
            None means clients are simulated with the same data quantity.
        alpha (float): The parameter for Dirichlet distribution simulation, applicable only when split_type is `dir`.
        public : The split method of public dataset
    Returns:
        :obj:`FederatedDataset`: Training dataset.
        :obj:`FederatedDataset`: Testing dataset.
        :obj:`FederatedDataset`: public dataset. # ADD
    """
    print(public)
    train_data, test_data, public_data, process_x, process_y, transform_train, transform_test = load_data(root,
                                                                                             dataset_name,
                                                                                             num_of_clients,
                                                                                             split_type,
                                                                                             min_size,
    ...
    public_data = FederatedTensorDataset(public_data,
                                       simulated=True,
                                       do_simulate=False,
                                       process_x=process_x,
                                       process_y=process_y,
                                       transform=transform_train)
    return train_data, test_data, public_data # 48000, 10000, 2000
