...
def non_iid_dirichlet(data_x, data_y, num_of_clients, alpha, min_size, x_dtype, y_dtype):
    ...
    while current_min_size < min_size:
        ...
        for k in range(num_class):
            idx_k = np.where(data_y == k)[0]  # label k index
            np.random.shuffle(idx_k)  # shuffle
...
def data_simulation(data_x, data_y, num_of_clients, data_distribution, weights=None, alpha=0.5, min_size=10,
                    class_per_client=1, public = "iid", stack_x=True):
    ...
    # SPLIT PUBLIC DATASET - iid sample
    if public == "iid":
        data_x, data_y = shuffle(data_x, data_y)
        num_of_public_data = 4000
        public_x = data_x[:num_of_public_data]
        public_y = data_y[:num_of_public_data]
        public_dataset = {}
        public_dataset['x'] = np.array(public_x).astype(x_dtype)
        public_dataset['y'] = np.array(public_y).astype(y_dtype)
        data_x = data_x[num_of_public_data:]
        data_y = data_y[num_of_public_data:]

    # SLPIT PUBLIC DATASET - noniid sample
    elif public == "class":
        n_train = data_x.shape[0]
        num_class = np.amax(data_y) + 1
        data_size = data_y.shape[0]
        num_of_public_data = 4000 # public dataset size
        num_of_each_class = int(data_size/num_class * 0.2)
        idx_batch = []

        # select class
        class_list = [i for i in range(num_class)]
        num_class = np.random.choice(class_list, int(num_class * 0.4), False)  # random select 4 class for public dataset
        print(len(num_class))

        for k in num_class:
            idx_k = np.where(data_y == k)[0]  # label k index
            np.random.shuffle(idx_k)  # shuffle
            pub_k_idx = np.random.choice(idx_k, num_of_each_class, False)
            idx_batch = idx_batch + pub_k_idx.tolist()

        public_dataset = {}
        public_dataset['x'] = np.array(data_x[idx_batch]).astype(x_dtype)
        public_dataset['y'] = np.array(data_y[idx_batch]).astype(y_dtype)
        # print(len(idx_batch))
        # print(data_x.shape)
        idx_to_keep = np.ones(data_x.shape[0], dtype=bool)
        idx_to_keep[idx_batch] = False
        data_x = data_x[idx_to_keep]
        data_y = np.delete(data_y, idx_batch)
    ...
    if weights is None:
        return group_client_list, group_federated_data, public_dataset
    ...
    for i in group_client_list:
        ...
        for j in range(len(input_lists)):
            ...
            clients.append(client_id)
            cur_key += 1
    return clients, federated_data, public_dataset
